...
const API_ENDPOINT = `${API_BASE_URL}/api/v1/books/search/text`;
const STATS_ENDPOINT = `${API_BASE_URL}/api/v1/books/stats/embeddings`;
...
function displayResults(data) {
    const { query, similar_books } = data;

    resultsTitle.textContent = `Results for "${query}"`;
    resultsCount.textContent = `Found ${similar_books.length} similar books`;
    ...
    resultsContainer.innerHTML = similar_books.map((item, index) => {
        const { book, similarity_score } = item;
...
function updateStats(stats) {
    totalBooks.textContent = stats.total_books.toLocaleString();
    embeddingsCount.textContent = stats.books_with_embeddings.toLocaleString();
    coveragePercent.textContent = `${stats.coverage_percentage}%`;
}
